		...
		if (!pause) {
			...
			if (snake.isDead()) {
				...
				if (life > 0) {
					...
					Game.GAME_OVER.init(container, game);
					GameOverState.setScore(snake.getScore());
					GameOverState.setHighScore(Util.compareScore("resources/highscore.text",
							snake.getScore()));
					out.init(Game.PLAY, Game.GAME_OVER);
					in.init(Game.PLAY, Game.GAME_OVER);
					
					game.enterState(Game.GAME_OVER.getID(), out, in);
				}
			...
			if (menuBox.isHover(input.getMouseX(), input.getMouseY()) && 
				...
				//go to menu screen
				//exit
				out.init(Game.PLAY, Game.MENU);
				in.init(Game.PLAY, Game.MENU);
